...
<template>
    <header
      class="w-full bg-gradient-to-br from-[#79cd37] via-[#b6e89c] to-[#3c9cbc] shadow-2xl fixed top-0 left-0 right-0 z-50 border-b-4 border-green-600 backdrop-blur-md"
    >
        ...
        <div class="flex h-20 justify-between items-center">
          ...
          <div class="flex items-center gap-3">
            ...
            <RouterLink to="/" class="flex-shrink-0">
              <h1
                class="text-2xl font-extrabold text-green-800 drop-shadow-lg tracking-wide lg:block hidden bg-white/80 px-4 py-2 rounded-lg"
              >
          ...
          >
            <span class="block w-7 h-1 bg-green-800 rounded transition-all duration-300" :class="mobileMenu ? 'rotate-45 translate-y-2' : ''"></span>
            <span class="block w-7 h-1 bg-green-800 rounded my-1 transition-all duration-300" :class="mobileMenu ? 'opacity-0' : ''"></span>
            <span class="block w-7 h-1 bg-green-800 rounded transition-all duration-300" :class="mobileMenu ? '-rotate-45 -translate-y-2' : ''"></span>
          </button>
        </div>
        <!-- Mobile Menu -->
        <transition name="fade">
          ...
          >
            <div class="flex flex-col items-center py-6 space-y-2">
              <button
                v-if="isHome"
                @click="emit('scrollToTop'); mobileMenu = false"
